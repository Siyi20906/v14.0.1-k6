...
const Phase6 = function(host1, ID_ORDER1, ID_OPER1,domain1 , username01, password01,username02, password02, cookie1) {
	...
	var obj;
	let bpasswordcorrect1 = false;
	let bpasswordcorrect2 = false;
	...
	{
		console.log('++++++++++phase6 : response is empty++++++++++++++++++++++++');
		return 0;
	}
	...
	while( !bpasswordcorrect1 )
	{
		...
		if( res.json().mobileScreen != null && res.json().mobileScreen != undefined )
		{
		   ...
		   {
			   bpasswordcorrect1 = true;
		   }
		   else if( JSON.stringify(res.json().mobileScreen.message).search("password") == -1 )
		   {
			   //print error, get out;
			   printerror("Phase6_1 General error: ", url, ID_ORDER, ID_OPER, res.json().mobileScreen.message);
			   //return 0;
		   }else
		   {
			   //password error
			   printerror("Phase6_1 password error: ", url, ID_ORDER, ID_OPER, res.json().mobileScreen.message);
		   }
		}
		else
		{
		...
		if( bpasswordcorrect1 )
		{
			while( !bpasswordcorrect2)
			{
				//submit username2
				...
				//check password issue
				if (res.json().mobileScreen != null && res.json().mobileScreen != undefined) {
					printerror("Phase6_wrong", url, ID_ORDER, ID_OPER, obj);
					if (res.json().mobileScreen.message == null || res.json().mobileScreen.message == undefined || res.json().mobileScreen.message == '') {
						bpasswordcorrect2 = true;
					}
					else if (JSON.stringify(res.json().mobileScreen.message).search("password") == -1) {
						//print error, get out;
						printerror("Phase6_2  General error: ", url, ID_ORDER, ID_OPER, res.json().mobileScreen.message);
						//return 0;
					} else {
						//password error
						printerror("Phase6_2 password error: ", url, ID_ORDER, ID_OPER, res.json().mobileScreen.message);
					}
				}
				else {
					//no error
